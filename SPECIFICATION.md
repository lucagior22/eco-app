...
> **Implementado (v4, 2026-08-18):** la detección dejó de hacerse con visión clásica local. `/api/pedal/detect` llama a **Gemini** (modelo multimodal) con el SDK `@google/genai`, mandando una sola captura; `scripts/detect_knobs.py` queda en el repo como evidencia del proceso pero ya no se invoca. El detector OpenCV tenía un techo medido de ~68 % de consistencia por foto con limitaciones estructurales —sesgo por perspectiva, umbrales calibrados a un solo pedal— y no resultaba usable en la práctica. Tres cambios acompañan la migración: **(a)** cada perilla se nombra con su **etiqueta impresa** en el panel ("Tone", "Level") cuando el modelo la lee, y con la posición ("Arriba izquierda") cuando no — algo que el pipeline OpenCV nunca pudo dar y que es la mejora de accesibilidad más grande de esta iteración; **(b)** la posición se narra con una **escala verbal de cinco escalones** (al mínimo / bajo / al medio / alto / al máximo), con la hora de reloj como detalle secundario entre paréntesis, porque un error de 15° cambiaba la hora reportada y la medición previa mostró que buena parte del error era de exactamente una hora; **(c)** la ráfaga de 5 capturas se reduce a una sola foto: sin votación que compensar, y medido que el modelo no aprovecha las capturas extra para calibrar su confianza, tres fotos costaban el triple de tokens con idéntico resultado. La **abstención explícita se mantiene**: el modelo devuelve un campo de confianza y, si es baja, la route fuerza `value: null` y la UI dice "no pude leerla con confianza". Esto es más necesario que antes, no menos: el detector clásico se abstenía por construcción, mientras que un modelo de lenguaje inventa un número con aplomo si no se lo instruye en contra. **Costo:** las fotos salen del dispositivo hacia Google y sin internet la pantalla no funciona (con mensaje narrado específico según el fallo: sin clave, cuota agotada o sin conexión). La precisión de lectura de ángulo **todavía no está medida** contra el pedal físico. Ver DECISIONS.md y `claude-docs/PEDAL.md`.
